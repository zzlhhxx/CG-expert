...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            sql = f"select id,source_id,aminer_id,profilePubsTotal from {self.table_name}  where id >%s  and is_zl=0  and is_ava=1 order by id asc limit 1000"
    ...
    def run(self):
        ...
        while not consumer_thread_exit:
            ...
            if row is False:
                # print("没有啦")
                break
            in_id = row[0]    #id,source_id,aminer_id,profilePubsTotal
            print(in_id)
            source_id = row[1]
            aminer_id=row[2]
            try_time = 0
