...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            sql = 'select id,aminer_id from paln_list_people where is_ava=1 and is_xm=0 and is_ids=1  order by  id asc limit 1000'
            cursor.execute(sql,)
            # cursor.execute(sql)
            ...
            if aminer_id:
                ...
                while try_time < 2:
                    try:
                        ...
                        url = f"https://backend.aminer.cn/api/v0/dataCenterPro/gateway/api/v3/project/person/v2?id={aminer_id}"
                        # he['Content-Type'] = 'application/json;charset=utf-8'
                        res = requests.get(url, headers=he, timeout=20, )
                        resp_data = res.json()
                        if resp_data['success']:
                            # print(aminer_id,data)
                            ...
                            xm_ids_ = ',,'.join(xm_ids)
                            xm_response = json.dumps(resp_data, ensure_ascii=False)

                            update_sql = f"update paln_list_people  set xm_response=%s,xm_ids=%s,is_xm=1 where id = %s"
                            cursor.execute(update_sql, (xm_response, xm_ids_, ids))
                            self.conn.commit()
                            with lock:
...
path_html=r'Y:/cg_expert_data/html/aminer_xm_detail_paln_list_people/'
#
YNewsTransTitleMain(
