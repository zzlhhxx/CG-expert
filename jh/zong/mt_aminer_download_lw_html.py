...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            sql = f"select id,aminer_id from {self.table_name}  where id >%s  and is_lw=0  and is_ava=1 and is_ids=1 order by id asc limit 1000"
...
class ConsumerThread(threading.Thread):
    ...
    def run(self):
        ...
        while not consumer_thread_exit:
            ...
            while 1: #下载多页
                try:
                    ...
                    if res.status_code == 200:
                        hitsTotal = data['data'][0]['data']['hitsTotal']
            ...
            update_sql = f"update paln_list_people  set lw_path=%s,is_lw=%s where id = %s"
...
path_html = r'Y:/cg_expert_data/html/aminer_lw_paln_list_people/'
