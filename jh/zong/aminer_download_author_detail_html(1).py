...
def mysql_db_conn(name='localhost',dbname='cg'):
    global conn
    if name =='localhost':
        # global conn
...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            sql = f"select id,f_id,xm_id from {self.table_name} where id>{ids} and is_download_path=0   order by  id asc limit 1000"
            cursor.execute(sql,)
            # cursor.execute(sql)
            mes_list = list(cursor.fetchall())  # ((id,f_id,xm_id)(id,f_id,xm_id))
            print(len(mes_list))
            if len(mes_list) >0 :
                ids = mes_list[-1][0]
            ...
            if xm_id:
                ...
                while try_time < 2:
                    try:
                        ...
                        url = "https://backend.aminer.cn/api/v0/dataCenterPro/gateway/api/v3/project/list/v2"
                        data = {"ids": [xm_id]
                                }
                        res = requests.post(url, data=json.dumps(data), timeout=20)
                        resp_data = res.json()
                        if res.status_code ==200:
                            path = path_html + str(f_id // 10000) + '/' + str(ids // 100) + '/'
                            if os.path.exists(path) is False:
                                os.makedirs(path)
                            dest_dir = os.path.join(path, str(ids)  + ".json")
                            with open(dest_dir, 'w', encoding='utf-8') as ff:
                                json.dump(resp_data, ff, ensure_ascii=False)
                            html_path = dest_dir.replace(path_html[0:1] + ":/", "")
                            # print(html_path)

                            update_sql = "update aminer_xm set xm_detail_path=%s,is_download_path=%s where id = %s"

                            cursor.execute(update_sql, (html_path, 1, ids))
                            self.conn.commit()
                            with lock:
...
path_html=r'Z:/cg_expert_data/html/aminer_xm_detail/'
#
YNewsTransTitleMain(
