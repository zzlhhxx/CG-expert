...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            sql = f"select id,aminer_id from {self.table_name}  where id >%s   and  is_download_html=0 and is_ava=1 and is_ids=1 order by id asc limit 1000"
...
path_html=r'Y:/cg_expert_data/html/aminer_html_paln_list_people/'   #下载存储的路径
