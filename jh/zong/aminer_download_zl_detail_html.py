...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            sql = f"select id,zl_id from {self.table_name}  where id >{ids}  and  is_donwload_path=0  order by id asc limit 1000"
            cursor.execute(sql,)
            # cursor.execute(sql)
...
class ConsumerThread(threading.Thread):
    ...
    def run(self):
        ...
        while not consumer_thread_exit:
            ...
            zl_id = row[1]
            try_time = 0
            url = f'https://backend.aminer.cn/api/v0/dataCenterPro/gateway/api/v3/patent/detail?id={zl_id}'
            while try_time < 2:
                try:
                    time.sleep(1)
