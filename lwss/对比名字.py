...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            # sql = f"-- select id,doi,aminer_detail_json,work_list_path,oa_name from {self.table_name} where  id >%s and aminer_id is null and oa_to_aminer=-4  and is_download=1 limit 1000"
            sql = f"select id,doi,aminer_detail_json,work_list_path,oa_name from {self.table_name} where  id=3 "
...
class ConsumerThread(threading.Thread):
    ...
    def run(self):
        ...
        while not consumer_thread_exit:
            ...
            # print(address)
            work_list_path = 'Z:/' + work_list_path
            # print(address)
            oa_data = open(work_list_path, 'r', encoding='utf-8').read()
            oa_d = json.loads(oa_data)
            results = oa_d['results']
            for asdas in results:
                if asdas.get("doi") and asdas.get("title"):
                    od_dio = asdas['doi'].replace("https://doi.org/", '')
                    if od_dio == doi:
                        continue
                    else:
                        authorships = asdas['authorships']
                        if len(authorships) >1:
                            lw_title=asdas['title']
                            update_='update oa_to_aminer set lw_title=%s,doi=%s,is_download=0 where id =%s'
                            cursor.execute(update_,(lw_title,od_dio,in_id))
                            self.conn.commit()
