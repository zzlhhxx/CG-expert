...
he = {
    ...
    'Authorization': "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MzUwOTYyODEsInRpbWVzdGFtcCI6MTczNTAwOTg4MSwidXNlcl9pZCI6IjY3NDUzNWVkNWZjNWJlOTNiMTlmY2Q3OCJ9.u0y5m8cP0vo3Zf3LQQEqipgtaeN6KVGQhEFh-aBhWI4"
}
...
class ProductThread(threading.Thread):
    ...
    def run(self):
        ...
        while not getValue('PRODUCT_EXIT'):
            be = time.time()
            sql = f"select id,xm_id, from {self.table_name}  where is_download_path=1 order by id asc limit 1"
...
class ConsumerThread(threading.Thread):
    ...
    def run(self):
        ...
        while not consumer_thread_exit:
            ...
            print(xm_id)
            try_time = 0
            url = f'https://datacenter.aminer.cn/gateway/open_platform/api/v3/paper/platform/details/not/contain/wos/by/id?id={xm_id}'
            ...
            #         print(res.text)
                    ...
                    #     update_sql = f"update {self.table_name} set html_path=%s,is_download_html=%s where id = %s"
...
def YNewsTransTitleMain(consumer_thread_size, product_thread_size, queue_max_size, proxy_host, db_host, db_name,
    ...
    # 创建并开始消费者，从指定的队列中获取连接地址，通过ip代理请求下载
    # time.sleep(100)
...
path_html=r'E:/cg_expert_data/html/aminer_html/'   #下载存储的路径
